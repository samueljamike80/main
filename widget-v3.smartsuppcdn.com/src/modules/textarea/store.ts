...
import { isMessageSpamming, isReachLinksLimit, replaceEmoticonsByEmojis } from './utils'
...
// TODO check if this a right place for all checks and file upload. Especially for auth form there is called after authForm directly only sendMessage method above
export const sendMessageWithAuthCheck = debounce(async () => {
	...
	if (get(cardsInProgress)) {
		warningMessage(get(t)('card.sendingInProgress'))
		return
	}
	if (isReachLinksLimit(get(messengerInputValue))) {
		warningMessage(get(t)('card.linksLimitReached'))
		return
	}
...
export const isMessageDuplicate = () => {
	...
	const typedMessage = get(messengerInputValue).trim()
	return (
